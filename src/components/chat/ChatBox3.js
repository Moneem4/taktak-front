...
function ChatBox3(props) {
  const {conversationId, index} = props
  const {closeChatBox} = useContext(UiContext)
  const {activeUser, getConversationById, sendMessage, messagesList3, setMessagesList3} = useContext(UserContext);
  ...
  useEffect(() => {
    getConversationById(conversationId, setConversationData).then((data) => {
      ...
      if(data.messages){
        setMessagesList3(data.messages)
      }
    });
  }, [props]);
  ...
  useEffect(() => {
    if(data) {
      setMessagesList3([...messagesList3, data.newMessages]);
    }
  },[data]);
      ...
      <div className="convesation-open">
        ...
        <div className="discussion-main" style={{height: 240, overflow: 'scroll'}}>
          <div className="discussion">
            {messagesList3 && messagesList3.map((message) => (
