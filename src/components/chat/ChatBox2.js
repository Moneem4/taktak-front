...
function ChatBox2(props) {
  ...
  const {activeUser, getConversationById, sendMessage, messagesList2, setMessagesList2} = useContext(UserContext);
  ...
  useEffect(() => {
    getConversationById(conversationId, setConversationData).then((data) => {
      ...
      if(data.messages){
        setMessagesList2(data.messages)
      }
    });
  }, [props]);
  ...
  useEffect(() => {
    if(data) {
      setMessagesList2([...messagesList2, data.newMessages]);
    }
  },[data]);
      ...
      <div className="convesation-open">
        ...
        <div className="discussion-main" style={{height: 240, overflow: 'scroll'}}>
          <div className="discussion">
            {messagesList2 && messagesList2.map((message) => (
